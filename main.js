...
async function osForm(x={}) {
  ...
  <div class="actions"><button class="secondary" id="osBack">Cancelar</button>${x.id&&x.status!=='Concluída'&&x.status!=='Cancelada'?'<button class="secondary" id="closeOS">Fechar OS</button>':''}<button class="primary" id="saveOS">${x.id?'Salvar alterações':'Criar OS'}</button></div></div>`
  ...
  const recalc=()=>{document.querySelector('#osValorServ').value=itens.reduce((s,i)=>s+(Number(i.quantidade)||0)*(Number(i.valor_unitario)||0),0).toFixed(2);document.querySelector('#osValorPecas').value=pecas.reduce((s,i)=>s+(Number(i.quantidade)||0)*(Number(i.valor_unitario)||0),0).toFixed(2)}
  ...
  const saveChildren=async(osId)=>{await supabase.from('itens_os').delete().eq('os_id',osId);await supabase.from('pecas_os').delete().eq('os_id',osId);const validI=itens.filter(i=>i.descricao&&Number(i.quantidade)>0).map(i=>({os_id:osId,servico_id:i.servico_id||null,descricao:i.descricao,quantidade:Number(i.quantidade),valor_unitario:Number(i.valor_unitario||0)}));const validP=pecas.filter(i=>i.descricao&&Number(i.quantidade)>0).map(i=>({os_id:osId,descricao:i.descricao,quantidade:Number(i.quantidade),valor_unitario:Number(i.valor_unitario||0)}));if(validI.length){const {error}=await supabase.from('itens_os').insert(validI);if(error)throw error}if(validP.length){const {error}=await supabase.from('pecas_os').insert(validP);if(error)throw error}}
